...
/**
 * A factory for easier testing concerning the {@link IBlockStore}. Created by
 * typus on 11/12/15.
 */
public class BlockStoreFactory {
	public static IBlockStore getBlockStore() throws BlockStoreException {
		return new LevelDbBlockStore(MockedIO.getDefaultBlockStoreFile());
	}
	
	public static void deleteBlockStore() {
		MockedIO.deleteDefaultBlockStoreFile();
	}
	...
	public static Block generateGatewayChain(Block block) {
		Gateway gateway = BlockStoreFactory.generateGateway();

		block = BlockStoreFactory.getRandomBlockChain(block);
	...
	public static List<Transaction> getConnectedTransactions() {
		...
		outPrev.setLockingScript("123456");
		String bytePrevOut = ByteArray.convertToString(outPrev.getConcatedOutput());

		Input inNew = new Input(3, bytePrevOut, 0);
		inNew.setUnlockingScript(EInputUnlockingScript.PUBLIC_KEY, lockingScript);
		Output outNew = new Output(2, publicKey, lockingScript);
