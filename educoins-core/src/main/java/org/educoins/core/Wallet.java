...
public class Wallet {
	...
	public Wallet(File file) {
		...
		try {
			...
			this.directoryKeyStorage = Paths.get(System.getProperty("user.home") + File.separator + "documents"
					+ File.separator + "educoins" + File.separator + "demo" + File.separator + "wallet");
	...
	// TODO [Vitali] Bad performance because the whole file will be checked over
	...
	/**
	 * At first each new private key and public key will be stored in the
	 * ".keys" file. before the public key will be returned. Moreover, the
	 * public key will be returned as string value, because it is really just a
	 * random number.
	 * 
	 * @return Each call of the method will create a new set of private and
	 *         public key set!
	 */
	// TODO[Vitali] Right now the private public key is stored in a file but
	// this can change, maybe???
	 ...
	 * because only the wallet is managing keys and verifications etc.
	 * 
	 ...
	 *              keys randomly using just a coin, pencil, and paper: toss a
	 *              coin 256 times and you have the binary digits of a random
	 *              private key you can use in a wallet. The public key can then
	...
	private class ECDSA {
		...
		/**
		 * Public/private key verification for the Elliptic Curve Digital
		 ...
		 * 
		 * @warning For verification purpose one need to initial the signature
		 *          each time.
		 * 
		 * @throws Exception
		 *             [Class ECDSA] The signature transaction hash value cannot
		 *             be null.
		 * 
		 */
		public boolean verifySignature(String message, byte[] signature, String publicKey) throws Exception {
			...
			if (message == null || signature == null) {
				throw new Exception(
						"EXCEPTION: [Class ECDSA] The signature or the transaction hash value cannot be null.");
			}
		...
		public byte[] getSignature(String publicKey, String message) throws Exception {
			...
			// Only ECPrivateKeySpec and PKCS8EncodedKeySpec supported for EC
			// private keys
			KeyFactory keyFactory = KeyFactory.getInstance(ECDSA);
		...
		// TODO[Vitali] Adub this to the ECDSA class is preaty goot... -> When
		// Time
		...
		// }
		//
		// public static String savePrivateKey(PrivateKey priv) throws
		// GeneralSecurityException {
		// KeyFactory fact = KeyFactory.getInstance("DSA");
