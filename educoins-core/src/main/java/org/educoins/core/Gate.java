...
public class Gate {
	...
	public String toString() {
		return Objects.toStringHelper(this).add("signatures", signatures).add("publicKey", publicKey)
				.add("message", message).add("signHere", signHere).toString();
	}
	...
	public byte[] concateExternGate() {
		byte[] message = ByteArray.convertFromString(getMessage(), 16);
		byte[] sig = ByteArray.convertFromString(getSignature(), 16);
		byte[] pubKey = ByteArray.convertFromString(getPublicKey(), 16);
		return ByteArray.concatByteArrays(sig, message, pubKey);
	}
