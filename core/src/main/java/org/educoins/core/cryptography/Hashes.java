...
public enum Hashes { // Verstehe nicht genau was die Klasse so macht????
	 ...
	 * Brauche ich das???
	 * 
	 * public static Hashes getHashFunction(byte id) { for (Hashes function :
	 * values()) { if (function.id == id) { return function; } } throw new
	 * IllegalArgumentException(String.format("illegal algorithm %d", id)); }
	 * 
